    ...
    def fit(self, x, y, w_init=None):
        ...
        for n in range(self.runs):
            ...
            for i in range(len(x)):
                gradient += (self.predict(x[i]) - y[i]) * x[i]
            gradient /= len(x)
            self.W -= self.eta * gradient.T
...
if __name__ == "__main__":
    ...
    for _ in range(10):
        ...
        all_models_basis1.append(model)
    # Here x and y contain last dataset:
    visualize_prediction_lines(x, y, all_models_basis1, basis1, "Logistic Regression Basis 1")
    ...
    for _ in range(10):
        ...
        model = LogisticRegressor(eta=eta, runs=runs)
        model.fit(x_transformed, y)
        all_models_basis2.append(model)
    # Here x and y contain last dataset:
    visualize_prediction_lines(x, y, all_models_basis2, basis2, "Logistic Regression Basis 2")
    ...
    for _ in range(10):
        ...
        model = LogisticRegressor(eta=eta, runs=runs)
        model.fit(x_transformed, y)
        all_models_basis3.append(model)
    # Here x and y contain last dataset:
    visualize_prediction_lines(x, y, all_models_basis3, basis3, "Logistic Regression Basis 3")
